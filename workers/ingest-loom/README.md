...
- **Public GraphQL** — `https://www.loom.com/graphql`. **Undocumented**; operation names drift. Owner, view/comment counts, transcript. Controlled by a kill switch.
...
| `LOOM_ENABLE_GRAPHQL` | no | `true` | Kill switch for the undocumented public GraphQL endpoint (owner info, view/comment counts, transcript). Set to `false` if Loom changes their schema — Core metadata still populates. |
...
| File | Purpose |
|---|---|
| `src/index.ts` | Worker entry point: declares the managed DB, four pacers, and both syncs. |
| `src/loom.ts` | `createLoomClient` — three independent fetchers (`fetchOEmbed`, `scrapeSharePage`, `fetchGraphQL`). Each catches failures and returns a tagged-union status. `parseVideoId` URL parser. Pure helpers `parseIso8601Duration`, `parseSharePageHtml`, `normalizeGraphqlResponse` are exported for tests. |
...
- **GraphQL is best-effort.** Owner info, view/comment counts, and transcripts use Loom's undocumented public GraphQL endpoint. Operation names drift — yt-dlp ships periodic fixes. If GraphQL starts failing, set `LOOM_ENABLE_GRAPHQL=false` to keep Core metadata flowing while you investigate.
- **Engagement metrics drift between backfills.** `loomDelta` only re-enriches a source row when it's edited in Notion; view/comment counts on Loom can change without touching the source row. Trigger `loomBackfill` periodically to refresh.
