...
| `workers/push-to-client` | **Building now** | Exposes a `pushToClient` worker tool with a `docType` discriminator that creates a single page in one of three canonical [Transformation Hub](https://www.notion.so/<transformation-hub-db-id>) destination databases per client: `Docs`, `Status Updates`, or `Deliverables`. Per-client internal-integration tokens (one `ntn_...` per client, scoped to all three destination DBs the client shares with the integration); no OAuth in MVP. Each destination is the canonical Transformation Hub schema plus a `Brain ID` rich_text augmentation we publish for idempotency — destination-side query, no state in our workspace. Push-once-read-only: a second push for the same `(brainId, docType)` returns `already_pushed` without writing. Relations (Docs `Project`, Status Updates `Event`) are not set in MVP — the client links them manually. `Presenter` on Status Updates is best-effort via email → `users.list` resolution; unresolved → property empty with a warning. Per-client `staging`/`production` mode; production pushes additionally require `allowProduction: true` on the tool call. Also exposes a `dispatchDraft` tool + `onDraftStatusChange` webhook implementing the [AI Drafts Trigger and Return](https://www.notion.so/notionstate/AI-Drafts-Trigger-and-Return-<ai-drafts-spec-page-id>) spec: a Notion database automation on the AI Drafts DB (`<ai-drafts-db-id>`) posts to the webhook on `Status` change, the dispatcher reads the draft + its `Artifact Category` and `Company` relations, calls `pushToClient` 1–2× (Client OS via `Company` → `COMPANY_PAGE_<ID>`; Notion State OS via the reserved `notionstate` client), and writes back `Status` + a Markdown `Location` to the draft. Partial failure leaves Status untouched (Brain ID dedup keeps retry safe). |
...
| `workers/ingest-loom` | **Building now** | Enriches Loom videos referenced from an existing Notion source DB (`<loom-source-db-id>`, `Video URL` property) into a managed Notion DB `Loom Videos`. One row per source page (`Source Page ID` = primary key) with title, thumbnail, duration, description, upload date, owner, comment count, and transcript. (View counts aren't reachable on Loom's public GraphQL surface — see `SCHEMA.md` known limitations.) Two syncs: `loomBackfill` (replace, manual), `loomDelta` (incremental, 5m). Enrichment uses four public Loom surfaces composed with graceful degradation — oEmbed + share-page OG/JSON-LD scrape (stable) + undocumented public GraphQL (kill switch `LOOM_ENABLE_GRAPHQL`) + signed-CloudFront transcript JSON whose URL GraphQL returns. Private / deleted videos kept as marker rows (`Sync Status = Private/Unavailable`) rather than skipped. No write-back to the source DB. |
...
- `workers/push-to-client` AI Drafts trigger today is a Notion DB-automation webhook (`onDraftStatusChange`); a future `worker.automation` GA migration would let the trigger live inside Notion Workers proper, with the same `dispatchDraft` internal helper behind it.
